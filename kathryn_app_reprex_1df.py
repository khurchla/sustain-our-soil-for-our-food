...
# access mapbox token
mapbox_access_token = 'pk.eyJ1IjoiYWRhbXNjaHJvZWRlciIsImEiOiJjanYwazBqaXgxazI2M3lsNm9uZWltNXhsIn0.nbQPgAuUeFZyIW1wLAM4Aw'
...
dffood = pd.DataFrame({'Reporter Countries': ['Afghanistan', 'Afghanistan', 'Albania', 'Albania', 'Benin', 'Benin'],
                      ...
                      'Item': ['Prunes', 'Raisins', 'Beans', 'Broccoli', 'Sheanuts', 'Sheanuts'],
                       'lon': [19.3256549835205, 19.3256549835205,
                               60.5537147521972, 60.6368370056152,
                               0.789650380611419, 0.872771501541137],
                       'lat': [42.1891021728515, 42.1063117980957,
                               32.9997024536132, 33.9103622436523,
                               10.4815368652343, 10.8126859664917],
                       })
...
def update_selected_trade_partner(selected_partner_country, selected_food):
    dfsoil_sub = dffood # copy of full dataframe containing geo points
    if bool(selected_partner_country): # if no country is selected, this is falsy so no filtering
        if selected_food is None: # if no food is selected,
