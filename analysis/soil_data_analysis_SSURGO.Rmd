---
title: "Soil Data Analysis SSURGO"
...
Soil data presented were derived from the 100+ year efforts of the National Cooperative Soil Survey, c/o USDA-NRCS

## Prepare environmnet

Install and load R packages
```{r,install package dependencies,echo=FALSE}
...
Load these packages to your current environment (or activated virtual environment)
```{r,load packages}
# load packages
library(aqp)
library(plyr)
...
## Warmup with this dataset 

Run an example plot showing soil color signatures
...
### Now have a look at the soil color schemes

```{r,soil colors series}
...
1. When was each survey area last exported to SSURGO? Show most recent at top.
```{r,query recency}
SDA_query("SELECT areasymbol, saverest FROM sacatalog WHERE areasymbol != 'US' ORDER BY saverest desc")
```

2. When was the most recent updated survey in Vermont, if any exist?
```{r,query last}
...
4. Get a list of map units that contain carbon as a component
```{r,query carbon}
q <- "SELECT
...
5. Have a look at the interpretations table
```{r,query interpretations example}
...
6. Get a list of distinct interpretation rule names and component names
*This query timed out* so I've commented it out below to prevent it from running
```{r,query list disctinct rules comps}
...
6.a. List of interpretation rule names and component names
*Try it without distinct
*Nope this timed out as well*
```{r,query list rules comps}
...
6.b. List of distinct interpretation rule names and component names
*Does removing filters speed it up? Nope this also timed out*
```{r,query list rules comps raw}
# SDA_query("SELECT DISTINCT c.compname AS Component_name, ci.mrulename AS Rule_name
# FROM legend AS l
